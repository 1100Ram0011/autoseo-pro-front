...
import Msg91WhatsAppPreview from '../template/Msg91WhatsAppPreview'
...
const parseTemplateVariables = (template) => {
  ...
  // Global Header
  if (template.header) {
    const hType = template.header.type?.toUpperCase();
    if (hType === 'TEXT' && template.header.text?.includes('{{1}}')) {
      vars.push({ key: 'header_text', label: 'Variable' });
    } else if (['IMAGE', 'VIDEO', 'DOCUMENT'].includes(hType)) {
      vars.push({ 
        key: 'header_url', 
        label: `Variable ${hType} URL`,
        optional: !!template.header.mediaUrl
      });
    }
  }
...
const WhatsAppPreview = ({ fromNumber, template, bodyValues }) => {
  ...
  const hType = template.header?.type?.toUpperCase()
  let headerType = 'None'
  if (hType === 'TEXT') headerType = 'Text'
  else if (hType === 'LOCATION') headerType = 'Location'
  else if (hType && hType !== 'NONE') headerType = 'Media'

  const renderText = (str, prefix = 'body') => {
    if (!str) return ''
    return str.replace(/\{\{(\d+)\}\}/g, (_, n) => {
      // For header_text, the prefix is 'header_text' and n is 1, so key is 'header_text' not 'header_text_1'
      // Wait, parseTemplateVariables pushes 'header_text' as the key for {{1}} in the header.
  ...
  const mapBtnType = (t) => {
    if (t === 'PHONE') return 'Call phone number'
    if (t === 'URL') return 'Visit website'
    if (t === 'QUICK_REPLY') return 'Quick Reply'
    return 'None'
  }
  ...
  const form = {
    ...
    header: headerType,
    headerMediaType: hType,
    headerText: hType === 'TEXT' ? renderText(template.header?.text || template.header?.value, 'header_text') : '',
    headerUrl: bodyValues['header_url'] || template.header?.mediaUrl || '',
    body: renderText(template.body, 'body'),
    footer: template.footer || '',
    buttons: (template.buttons || []).map(b => ({ ...b, type: mapBtnType(b.type) })),
    carouselCards: template.carouselCards?.map((card, idx) => ({
      ...card,
      headerMediaType: card.header?.type?.toUpperCase(),
      headerUrl: bodyValues[`card_${idx}_header_url`] || card.header?.mediaUrl || '',
      body: renderText(card.body, `card_${idx}_body`)
    })) || [],
    carouselButton1Type: mapBtnType(template.carouselCards?.[0]?.buttons?.[0]?.type),
    carouselButton2Type: mapBtnType(template.carouselCards?.[0]?.buttons?.[1]?.type),
  }
  ...
  return (
    <div className="w-full overflow-hidden rounded-[2rem] border-[6px] border-gray-900 bg-[#efeae2] shadow-2xl h-[520px] flex flex-col relative mx-auto max-w-[300px]">
      ...
      >
        <div className="absolute inset-0 p-4 pt-6 overflow-y-auto scrollbar-hide flex flex-col justify-end">
            <Msg91WhatsAppPreview form={form} hideShell={true} />
        </div>
      </div>
    </div>
  )
...
export default function Msg91WaCampaignModal({
  ...
  const {
    ...
    refetch,
  } = useMsg91GetTemplatesQuery(fromNumber, { skip: !fromNumber || !open })
  ...
  const handleSend = async () => {
    ...
    try {
      const payload = {
        ...
        quickReplyResponses: Object.keys(quickReplyResponses).length > 0 ? quickReplyResponses : undefined,
      }
  ...
  return (
    <>
      ...
      >
        {/* Modal */}
        <div className="modal-anim flex max-h-[90vh] w-full max-w-7xl flex-col overflow-hidden rounded-2xl bg-[var(--app-pages-bg)] shadow-2xl ">
          <div className="scrollbar flex flex-1 flex-col overflow-hidden border border-[var(--app-pages-border)] rounded-2xl ">
            {/* Header */}
            <div className="flex shrink-0 items-center justify-between border-b border-[var(--app-pages-border)] bg-[var(--app-pages-bg)] px-6 py-4 ">
              <div>
                <h2 className="m-0 font-sans text-lg font-bold text-[var(--app-pages-text)]">
                  WhatsApp Campaign 
                </h2>
                {step === 2 && (
                  <p className="m-0 mt-1 font-sans text-[13px] text-[var(--app-pages-text)] opacity-80">
              ...
              <button
                onClick={onClose}
                className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg border-none bg-transparent text-[var(--app-pages-text)] transition-colors hover:bg-gray-100 dark:hover:bg-zinc-800"
              >
                <svg
                  width="16"
                  height="16"
            ...
            {/* Footer */}
            {!success && (
              <div className="flex shrink-0 items-center justify-end gap-3 border-t border-[var(--app-pages-border)] bg-[var(--app-pages-bg)] px-6 py-4 ">
                {step === 2 && (
                  <button
                    onClick={() => {
                      setStep(1)
                    ...
                    className="cursor-pointer rounded-lg border border-[var(--app-pages-border)] bg-[var(--app-pages-bg)] px-5 py-2 text-[13px] font-medium text-[var(--app-pages-text)] transition-all hover:bg-gray-50 dark:hover:bg-zinc-800 "
                  >
                    ← Back
                  </button>
                )}
                ...
                {step === 1 ? (
                  <button
                    ...
                    className="cursor-pointer rounded-lg px-6 py-2 text-[13px] font-semibold text-[var(--app-profile-btn-text)] bg-[var(--app-profile-btn-bg)] transition-all hover:opacity-90 active:scale-95 shadow-sm"
                  >
                    Review & Send →
                  </button>
                ) : (
                  <button
                    onClick={handleSend}
                    disabled={sending}
                    className={`flex items-center gap-2 rounded-lg px-6 py-2 text-[13px] font-semibold text-[var(--app-profile-btn-text)] bg-[var(--app-profile-btn-bg)] transition-all hover:opacity-90 shadow-sm`}
                  >
                    {sending && (
                      <svg
                        width="16"
