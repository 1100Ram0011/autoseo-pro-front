...
import { PhoneIcon, ExternalLinkIcon, ReplyIcon, ImageIcon } from '@/components/Msg91/whatsappCampaign/components/ui/Msg91WaIcons'
...
// ─── Main Preview ─────────────────────────────────────────────────────────────
const MetaWhatsAppPreview = ({ form, hideShell = false }) => {
    ...
    const renderButtons = () => {
        ...
        ctaButtons.forEach(b => {
            if (b.type === 'PHONE_NUMBER' || b.type === 'CALL_PHONE' || b.type === 'PHONE')
                all.push({ label: b.text || 'Call Phone Number', icon: <PhoneIcon size={13} /> })
            if (b.type === 'URL' || b.type === 'VISIT_WEBSITE')
                all.push({ label: b.text || 'Visit Website', icon: <ExternalLinkIcon size={13} /> })
            if (b.type === 'COPY_CODE')
                all.push({ label: b.text || 'Copy Code', icon: <ExternalLinkIcon size={13} /> })
        })
        qrButtons.forEach(b => {
    ...
    if (category === 'MARKETING' && marketingType === 'PRODUCT' && productFormat === 'CATALOGUE') {
        return (
            <PreviewShell hideShell={hideShell}>
    ...
    if (category === 'MARKETING' && marketingType === 'PRODUCT' && productFormat === 'MULTI_PRODUCT') {
        return (
            <PreviewShell hideShell={hideShell}>
    ...
    // ── Marketing > Carousel ───────────────────────────────────────────────────
    if (category === 'MARKETING' && marketingType === 'CAROUSEL') {
        const btns = []
        if (carouselButton1Type && carouselButton1Type !== 'NONE')
            btns.push({ label: 'Button 1', icon: (carouselButton1Type === 'PHONE_NUMBER' || carouselButton1Type === 'CALL_PHONE') ? <PhoneIcon size={12} /> : (carouselButton1Type === 'URL' || carouselButton1Type === 'VISIT_WEBSITE') ? <ExternalLinkIcon size={12} /> : <ReplyIcon size={12} /> })
        if (carouselButton2Type && carouselButton2Type !== 'NONE')
            btns.push({ label: 'Button 2', icon: (carouselButton2Type === 'PHONE_NUMBER' || carouselButton2Type === 'CALL_PHONE') ? <PhoneIcon size={12} /> : (carouselButton2Type === 'URL' || carouselButton2Type === 'VISIT_WEBSITE') ? <ExternalLinkIcon size={12} /> : <ReplyIcon size={12} /> })
...
export default MetaWhatsAppPreview
